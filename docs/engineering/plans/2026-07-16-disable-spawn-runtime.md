...
> **For agentic workers:** REQUIRED SUB-SKILL: Use mattpocock:subagent-driven-development (recommended) or mattpocock:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
`docs/engineering/specs/2026-07-16-disable-spawn-runtime-design.md`.
...
```bash
SPAWN_BOUNDARY_SNAPSHOT_ID="$(rtk jq -r 'select(.session_id == "disable-spawn-runtime" and .phase == "pre" and .action == "exclude spawn runtime from production CLI") | .snapshot_id' .omniskills-legacy/snapshots.jsonl | rtk tail -n 1)"
rtk sh /Users/roy/.agents/skills/pony-trail/scripts/snapshot_change.sh \
  --session-id disable-spawn-runtime \
  post \
