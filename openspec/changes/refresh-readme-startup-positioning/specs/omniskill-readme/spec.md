# Omniskill README Specification

## ADDED Requirements

### Requirement: README Leads With Startup Team Positioning

The README SHALL lead with the current Omniskill product promise.

#### Scenario: reader opens the README

- **WHEN** a reader opens `README.md`
- **THEN** the first screen presents Omniskill with the headline
  "Power your ability."
- **AND** it describes Omniskill as an installable workflow skill tree
...
- **AND** it includes the current `omniskill loop` command surface
...
- **AND** it mentions Matt Pocock skills
...
- **THEN** it references a repo-local asset based on the provided startup role
  registry screenshot
- **AND** it does not embed `assets/diagrams/omniskill-how-it-works.svg`
- **AND** it does not embed `assets/diagrams/omniskill-install-sequence.svg`
